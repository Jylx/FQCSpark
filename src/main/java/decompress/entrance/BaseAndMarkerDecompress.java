...
public class BaseAndMarkerDecompress {
    ...
    public static Ref_base refSequenceExtraction(String refFileName) throws IOException {
        String str;
        char[] cha;
        int _seq_code_len = 0/*, _ref_low_len = 1, letters_len = 0*/;
    ...
    private static void readFile(String filePath) throws Exception {
        ...
        if (names == null)  {
            throw new Exception(filePath + "文件夹下没有文件");
        }
        Arrays.sort(names);
        for (String a : names) {
            seqPath.add(a);
    ...
    private static void readOtherData2(BufferedReader br) {
        ...
        try {
            ...
            line = br.readLine();
            assert "*".equals(line);
            lineLensSize = Integer.parseInt(br.readLine());
            if (lineLensSize > 0) {
                lineLens = new int[lineLensSize];
                numOfSameLineLens = new int[lineLensSize];
                readPositionRangeData2(br, lineLensSize, BaseAndMarkerDecompress.lineLens, BaseAndMarkerDecompress.numOfSameLineLens);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static void writeSecondLine(BufferedWriter bw, int _seqnum) throws IOException {
        int prePos = 0;
        if(spe_cha_len > 0) {
        ...
        prePos = 0;
        for(int i = 0; i < lineLensSize; i++) {
            for(int k = 0; k < numOfSameLineLens[i]; k++) {
                bw.write(sb1.substring(prePos, prePos+lineLens[i]));
                bw.newLine();
                prePos += lineLens[i];
            }
    ...
    private static void readFirstLine(BufferedReader br) throws IOException {
        ...
        num0List = new ArrayList<>(linelen/4);
        for(int i = 0; i < num; i++) {
            String l = br.readLine();
            String[] split = l.split("\\s+");
            int baseNum = Integer.parseInt(split[0]);
            int len = Integer.parseInt(split[1]);

            for(int k = 0; k < len+1; k++) {
                num0List.add(baseNum++);
            }
        ...
        assert "*".equals(line);
        num = Integer.parseInt(br.readLine());
        name1List = new ArrayList<>(linelen/4);
        for(int i = 0; i < num; i++) {
            String l = br.readLine();
            String[] split = l.split("\\s+");
            int len = Integer.parseInt(split[1]);
        ...
        assert "*".equals(line);
        num = Integer.parseInt(br.readLine());
        num1List = new ArrayList<>(linelen/4);
        for(int i = 0; i < num; i++) {
            String l = br.readLine();
            String[] split = l.split("\\s+");
            int tnum = Integer.parseInt(split[0]);
            int len = Integer.parseInt(split[1]);
        ...
        assert "*".equals(line);
        num = Integer.parseInt(br.readLine());
        num2List = new ArrayList<>(linelen/4);
        for(int i = 0; i < num; i++) {
            String l = br.readLine();
            String[] split = l.split("\\s+");
            int tnum = Integer.parseInt(split[0]);
        ...
        assert "*".equals(line);
        num = Integer.parseInt(br.readLine());
        num5List = new ArrayList<>(linelen/4);
        for(int i = 0; i < num; i++) {
            String l = br.readLine();
            String[] split = l.split("\\s+");
            int tnum = Integer.parseInt(split[0]);
    ...
    private static void writeFirstLine(List<String> nameList, List<Integer> num0List, List<String> name1List,
        ...
        assert nameList.size() == linelen/4 && nameList.size() == num0List.size() && num0List.size() == name1List.size() &&
                name1List.size() == num1List.size() && num1List.size() == num2List.size() &&
                num2List.size() == num3List.size() && num3List.size() == num4List.size() && num4List.size() == num5List.size();
        for(int i = 0; i < nameList.size(); i++) {
            bw.write(nameList.get(i));
            bw.write(".");
            bw.write(String.valueOf(num0List.get(i)));
            bw.write(" ");
            bw.write(name1List.get(i));
            bw.write(":");
            bw.write(String.valueOf(num1List.get(i)));
            bw.write(":");
            bw.write(String.valueOf(num2List.get(i)));
            bw.write(":");
            bw.write(String.valueOf(num3List.get(i)));
            bw.write(":");
            bw.write(String.valueOf(num4List.get(i)));
            bw.write("/");
            bw.write(String.valueOf(num5List.get(i)));
            bw.newLine();
        }
        bw.flush();
        bw.close();
